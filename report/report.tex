...
In the Battle of the Sexes, all models chose Opera---the first-listed
action---yielding exploitability $\varepsilon \approx 0.667$ against the mixed
NE opponent, consistent with Schelling's focal-point prediction.
...
We observe strong equality bias in the asymmetric game.
Both Haiku ($\varepsilon = 30.0$) and Sonnet ($\varepsilon = 20.0$) demanded
50 despite the NBS prescribing a demand of 60 given Row's outside option of 30.
Sonnet's chain-of-thought explicitly reasoned about the payoff matrix yet still
anchored to the equal-split as a focal point, overriding the correct NBS.
GPT-mini matched the NE ($\varepsilon = 0$), likely by pattern-matching the
payoff structure rather than via social preference.
...
($\bar\varepsilon = 2.931$), driven largely by Haiku's large deviation on Nash
Bargaining Asymmetric ($\varepsilon = 30.0$).
Sonnet outperformed both on Stag Hunt ($\varepsilon = 0$ vs.\ 4.0) but
underperformed on asymmetric bargaining due to stronger equality bias.
